...
Validation is intentionally conservative. A public fixture passes 13 ontology, matching, and cartographic-safety cases. A local QGIS 3.40.5 run and public QGIS 3.44.13 CI each pass 84 metric and integration tests. The indexed 100,000-feature benchmark completed in 27.28 s with 164.19 MiB peak memory locally and 3.24 s with 250.70 MiB in CI. The benchmark measures candidate generation, not end-to-end nationwide processing or archaeological accuracy.
...
Before submission, a disclosure-reviewed aggregate record of that workflow must add the exact plugin version, input counts, candidate counts, and human corrections. The 300-pair blinded single-reviewer pilot, two further anonymized workflows, and an external GIS installation test remain pending. Until those release gates are complete, the paper does not claim validated archaeological accuracy, quantified labour savings, or cross-institutional generalization, and it is **not ready for JOSS submission**.
